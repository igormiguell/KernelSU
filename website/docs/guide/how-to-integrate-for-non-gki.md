...
If you are able to build a bootable kernel, there are two ways to integrate KernelSU into the kernel source code:
...
Then, you should check if kprobe is enabled in your kernel config. If it's not, add these configs to it:
...
Keep in mind that, on some devices, your defconfig may be located in `arch/arm64/configs` or in other cases, it may be in `arch/arm64/configs/vendor/your_defconfig`. Regardless of the defconfig you're using, make sure to enable `CONFIG_KSU` with `y` to enable or `n` to disable it. For example, if you choose to enable it, your defconfig should contain the following string:
